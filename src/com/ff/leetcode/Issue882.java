...
//        System.out.println(Utils.toString(graph));
        ...
        int nextNode = findNextNode(graph, processed, n);
        while (nextNode != INF) {
            for (int i = 1; i < graph[nextNode].length; i++) {
                graph[0][i] = Math.min(graph[0][i], graph[0][nextNode] + graph[nextNode][i]);
                graph[i][0] = graph[i][0];
            }
            processed[nextNode] = 1;
            nextNode = findNextNode(graph, processed, n);
        }

//      System.out.println(Utils.toString(graph));
        int total = 0;
        for (int i = 0; i < edges.length; i++) {
            int small = Math.min(graph[0][edges[i][0]], graph[0][edges[i][1]]);
            int big = Math.max(graph[0][edges[i][0]], graph[0][edges[i][1]]);
            int count = edges[i][2];


            if (maxMoves >= big) {
                total = total + Math.min(count, maxMoves - big + maxMoves - small);
            } else if (small <= maxMoves) {
                total = total + Math.min(count, maxMoves - small);
            }

            //            System.out.println("small " + small + "  count " + count + " " + Math.min(count, maxMoves - small));
        }

        for (int i = 0; i < graph[0].length; i++) {
            if (graph[0][i] <= maxMoves) {
                total++;
            }
        }
    ...
    int findNextNode(int[][] graph, int[] processed, int n) {
        int min = INF;
        int nextNode = INF;
        for (int i = 1; i < n; i++) {
            if (graph[0][i] < min && processed[i] != 1) {
                min = graph[0][i];
                nextNode = i;
            }
        }
